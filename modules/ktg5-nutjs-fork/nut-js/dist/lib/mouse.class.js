...
class MouseClass {
     ...
     */
    async setPosition(target, relative = false) {
        ...
        try {
            await this.providerRegistry.getMouse().setMousePosition(target, relative);
            return this;
        }
        catch (e) {
            this.providerRegistry.getLogProvider().error(e);
     ...
     * @param {boolean} relative If the mouse movement should be a relative movement
     ...
     */
    async move(path, relative, movementType = mouse_movement_function_1.linear) {
        try {
            ...
            for (let idx = 0; idx < pathSteps.length; ++idx) {
                ...
                await this.setPosition(node, relative);
            }
            return this;
        }
        catch (e) {
